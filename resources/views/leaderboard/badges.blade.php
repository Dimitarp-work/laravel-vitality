                    ...
                    <div class="flex justify-center gap-12 mb-8">
                        @foreach ($topThree as $index => $user)
                            ...
                            <div class="text-center">
                                ...
                                <div class="text-sm text-gray-500">{{ $user['badge_count'] }} Badges</div>
                    ...
                    <div class="space-y-3">
                        @foreach ($users as $index => $user)
                            @php
                                $initials = strtoupper(substr(explode(' ', $user['name'])[0] ?? '', 0, 1) . substr(explode(' ', $user['name'])[1] ?? '', 0, 1));
                            @endphp
                            <div class="flex justify-between items-center bg-gray-50 px-4 py-3 rounded-md hover:bg-gray-100 transition-colors duration-200">
                                ...
                                <div class="text-gray-700 font-medium">{{ $user['badge_count'] }} Badges</div>
                            </div>
                        @endforeach
                    </div>
                </div>
