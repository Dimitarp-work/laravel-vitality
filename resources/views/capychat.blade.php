        ...
        @foreach($messages as $msg)
            @if($msg->sender === 'user')
                <div class="flex justify-end mb-1">
                    <div class='bg-pink-400 text-white rounded-lg px-4 py-2 max-w-xs text-right'>{{ $msg->message }}</div>
                </div>
            @else
                <div class="flex justify-start mb-1">
                    <div class='flex items-end gap-2'><span class='text-2xl'>🦫</span><div class='bg-white border border-pink-100 text-pink-900 rounded-lg px-4 py-2 max-w-xs'>{{ $msg->message }}</div></div>
                </div>
            @endif
        @endforeach
    </div>
